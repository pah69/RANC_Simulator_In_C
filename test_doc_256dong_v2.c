...
typedef struct {
    uint8_t synapse_connection[256]; // 256 bits (32 bytes)
...
void processLine(const char *bitStream, CSRAM *csram) {
    ...
    for(int i = 0;i<256;i++){
        csram->synapse_connection[i]=bitStream[i]-'0';
    }
    start += 256;
...
int main() {
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        // Xử lý từng dòng
        int k =0;
        processLine(line, &csram[0][k]);

        // In các giá trị trong cấu trúc để kiểm tra
       // printf("Synapse Connection: ");
        for (int i = 0; i < 32; i++) {
            printf("%02X", (unsigned char)csram[0][k].synapse_connection[i]);
        }
       // printf("\n");
